...
私聊时即可使用，不需要 Premium。键盘提供余额、盈亏、持仓、状态、策略、手动交易、停止、
恢复和重启按钮；每次按钮回复都会再次附带键盘。程序还会通过 `setMyCommands` 和
...
5 分钟窗口生效。实盘 Telegram 选择器提供默认的 `reversal_v11`、首阶段分别
要求连续三个或四个同向窗口的 `reversal_v11_three_streak` 和
`reversal_v11_four_streak` 和 `fair_value_edge`；`smart_score` 保持纸面专用。
反转轮次尚未结束时，切换会延后到整轮结束，避免中途遗留递增状态。选择“恢复启动策略”
可清除 Telegram 策略覆盖，回到启动命令指定的策略。

`reversal_v11` 在新窗口取得首个有效 Price to Beat 后，用相邻窗口的开盘边界价立即
推导刚结束窗口的结果（后一个边界价不低于前一个为 UP，否则为 DOWN）。完成态只快速
尝试一次；尚未发布或请求失败就立即采用相邻 Price to Beat，不再等待 Gamma
把 `outcomePrices` 更新为正式 1/0。每个边界结果都会进入待复核队列；Gamma 后续发布
正式结果后才核对，一致则记录通过，不一致则只暂停当前窗口并通知，不设置全局暂停。
`reversal_v11` 共十五阶段：第1阶段计划2份；第2～4阶段分别以4/8/16份为下限，
若仍不足以追回本轮此前累计实际亏损，则动态增加份数；第5～15阶段直接按追回累计
实际亏损的100%计算份数。若实际可用余额不足，则在满足交易所最小可成交金额时按可用余额
尽量下单；低于最小额时本窗口不下单。所有阶段均取消动态概率、edge、
开盘穿越和公允价值过滤，也不设置16份恢复上限；
因此110 pUSD不是第2～15阶段的实际本金硬上限。
连续十五次失败后结束本轮并锁定原趋势，直到真实反转后才允许重新触发。策略不设置盈利暂停，
完成一轮后继续滚动判断后续窗口。状态保存在
`data/reversal_v11_state.json`，窗口结算继续复用
公允价值策略的盈利/亏损模板，并同时向 Telegram 和 Discord 推送。

`reversal_v11_three_streak` 和 `reversal_v11_four_streak` 与上述参数、十五阶段递增、
累计亏损追回、余额不足处理、极端波动过滤及通知规则完全一致，唯一差异是新一轮
第1阶段必须分别由紧邻当前窗口的连续三个或四个同向结算窗口触发；
进入轮次后的第2～15阶段仍按原策略继续执行。

实盘启动自检要求 CLOB 认证、至少足够执行下一阶段的可用余额（新轮次为第1阶段2 pUSD）、
...
`src/spread_market_maker.py` 提供独立的五分钟盘口价差决策引擎，并由
`src/spread_maker_runtime.py` 接入实盘 Post-Only 挂单、撤单、部分成交/余额核对和崩溃恢复。
它只在剩余270～90秒、双边spread至少0.03、双边bid合计不高于0.99且盘口、波动和
深度正常时，同时生成UP/DOWN两笔Post-Only GTC买单。单侧成交后立即挂高0.03的
Post-Only止盈卖单；另一腿最多等待4秒。库存持有超过20秒、价格逆向0.05、进入90秒
截止区或剩余30秒时撤销挂单并用FAK退出。默认每侧5份（满足BTC五分钟市场最小订单量），
窗口承诺本金最多5.00 pUSD。未知挂单、未知持仓或提交结果不确定时会暂停当前窗口。
Maker返佣不计入策略盈利判断。
...
记录告警但仍使用 Price to Beat。除 `reversal_v11` 外，程序至少读取两次相同值且保持
5 秒才采用；`reversal_v11` 为缩短触发延迟，首个有效值立即采用。读取失败时
